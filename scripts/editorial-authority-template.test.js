...
test('The three signature templates have stable preview, PDF, and chooser mappings', () => {
  ...
  const firstTemplateId = pageSource.match(/const ALL_TEMPLATES = \[[\s\S]*?\{ id: '([^']+)'/)?.[1];
  assert.equal(firstTemplateId, 'editorial-authority');
  assert.match(pageSource, /const FEATURED_TEMPLATE_IDS = new Set\(\['editorial-authority', 'technical-signal', 'brutalist-voltage'\]\)/);
