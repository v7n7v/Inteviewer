    ...
    <>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40">
        <button
          ...
          className="group flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800/80 border border-white/10 backdrop-blur-xl hover:border-white/20 transition-all"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              <div className="mx-4 rounded-2xl bg-slate-900/95 border border-white/10 shadow-2xl shadow-black/50 overflow-hidden backdrop-blur-xl">
                {/* Search Input */}
                <div className="flex items-center gap-3 p-4 border-b border-white/10">
                  ...
                  />
                  <kbd className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-400 font-mono">ESC</kbd>
                </div>
                ...
                {/* Footer */}
                <div className="flex items-center justify-between p-3 border-t border-white/10 bg-slate-800/50">
                  ...
                  <span className="text-xs text-slate-500">TalentConsulting.io</span>
                </div>
              </div>
            </motion.div>
          </>
...
function CommandItem({
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left transition-colors ${isSelected ? 'bg-white/10 text-white' : 'text-slate-300 hover:bg-white/5'
        }`}
    >
      <span className="text-xl">{command.icon}</span>
      <div className="flex-1 min-w-0">
        <p className="font-medium truncate">{command.label}</p>
        <p className="text-sm text-slate-500 truncate">{command.description}</p>
      </div>
      {isSelected && (
        <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
