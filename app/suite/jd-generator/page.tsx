    ...
    <div className="min-h-screen p-6 lg:p-8">
      ...
      >
        <div className="absolute inset-0 opacity-20">
          ...
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-pink-500/30 rounded-full blur-3xl" />
        </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="p-6 space-y-4">
                    <div>
                      ...
                      <input type="text" value={roleTitle} onChange={(e) => setRoleTitle(e.target.value)}
                        className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white text-lg focus:border-purple-500/50 focus:outline-none"
                    ...
                    <div>
                      ...
                      <textarea rows={3} value={teamContext} onChange={(e) => setTeamContext(e.target.value)}
                        className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-slate-300 focus:border-purple-500/50 focus:outline-none resize-none"
                    ...
                    <div className="flex items-center gap-4 pt-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={includeCompensation} onChange={(e) => setIncludeCompensation(e.target.checked)}
                          className="w-5 h-5 rounded bg-black/30 border border-white/20 checked:bg-purple-500"
                        />
                    ...
                    {JD_STYLES.map((style) => (
                      <button key={style.id} onClick={() => setSelectedStyle(style)}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          selectedStyle.id === style.id ? 'border-purple-400 bg-purple-500/10' : 'border-white/10 hover:border-white/30'
                        }`}
                      >
                ...
                <button onClick={generateJD} disabled={isLoading || !roleTitle.trim()}
                  className="w-full py-5 rounded-2xl font-bold text-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:shadow-lg hover:shadow-purple-500/25 transition-all disabled:opacity-50"
                >
                ...
                <div className="rounded-2xl bg-gradient-to-br from-slate-900/80 to-slate-800/50 border border-white/10 overflow-hidden">
                  <div className="p-6 border-b border-white/10 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={saveTemplate} className="px-3 py-1.5 rounded-lg bg-purple-500/20 text-sm text-purple-300 hover:bg-purple-500/30">💾 Save</button>
                    </div>
                  </div>
                  <div className="p-6">
                    <textarea
                      ...
                      rows={25}
                      className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-slate-300 focus:border-purple-500/50 focus:outline-none resize-none font-mono text-sm leading-relaxed"
                    />
                  </div>
                </div>
                ...
                <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
                  ...
                  <div className="space-y-2">
                    <button onClick={copyToClipboard}
                      className="w-full py-3 rounded-xl font-semibold bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:shadow-lg transition-all"
